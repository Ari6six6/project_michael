...
def build_protocol() -> str:
    ...
    return "\n".join([
        ...
        "",
        "THREE QUESTIONS — answer these before acting on any task:",
        "1. What is the target?  (Which external system, app, or API am I working with?)",
        "2. What is the specific process?  (Which flow, endpoint group, or feature?)",
        "3. What is the point?  (What concrete outcome does the user want?)",
        "",
        "If working with an external system: call load_model(name, version) immediately.",
        "If no model exists, check targets/ for recon notes. If those are missing too,",
        "run recon_passive(target=...) before writing any code. Never guess API structure",
        "when a model or recon artifact is available.",
        "",
        "FULL TOOL ACCESS:",
        ...
        "",
        "APP MODELS:",
        "models/<name>-<version>.json contains ground-truth API shape synthesized from",
        "recon: base_url, auth pattern, endpoint signatures, stack, and notes.",
        "Call load_model(name, version) to retrieve it — auto-executes, no confirmation.",
        "Always prefer a model over training-data guesses about API structure.",
        "To build a model: read targets/ recon notes, call source_map if version is",
        "confirmed, then write_file the JSON to models/<name>-<version>.json.",
        "",
